...
<head>
    <meta charset="utf-8">
    <title>Documento Privado de Compra - {{ $compra->id }}</title>
    <style>
        @page {
            margin: 15px;
            size: letter;
...
<body>
    ...
    <div class="numero-documento">
        No. {{ str_pad($compra->id, 6, '0', STR_PAD_LEFT) }}
    </div>
    ...
    <div class="clausula">
        <span class="clausula-titulo">PRIMERA.-</span> Yo <span class="datos-cliente">{{ strtoupper($compra->cliente->nombre) }}</span> con C.I.: <span class="datos-cliente">{{ $compra->cliente->numero_documento }}</span><br>
        Domicilio: <span class="datos-cliente">{{ $compra->cliente->direccion ?? 'NO ESPECIFICADO' }}</span> Telf.: <span class="datos-cliente">{{ $compra->cliente->telefono_1 ?? 'NO ESPECIFICADO' }}</span><br>
    ...
    <div class="fecha-firma">
        Santa Cruz, <span class="linea-fecha">{{ $compra->fecha_compra->format('d') }}</span> de <span class="linea-fecha">{{ mesEnEspanol($compra->fecha_compra->format('n')) }}</span> del <span class="linea-fecha">{{ $compra->fecha_compra->format('Y') }}</span>
    </div>
    ...
    <div class="firma-section">
        <div class="firma-container">
            ...
            <div class="firma-box">
                <div class="linea-firma"></div>
                <div class="firma-texto">
                    <strong>{{ strtoupper($compra->cliente->nombre) }}</strong><br>
                    C.I.: {{ $compra->cliente->numero_documento }}<br>
                    VENDEDOR
                </div>
            </div>
        </div>
