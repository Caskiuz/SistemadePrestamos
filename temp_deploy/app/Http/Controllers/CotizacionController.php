...
class CotizacionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query->where(function ($q) use ($busqueda) {
            $q->where('cod_cotizar', 'like', "%$busqueda%")
    ...
    function generarCodigo() {
        ...
        if ($ultima_Cotizacion) {
            $ultimoNumero = (int) Str::after($ultima_Cotizacion->cod_cotizar, 'COT-00');
            $codigo = 'COT-00' . str_pad($ultimoNumero + 1, 4, '0', STR_PAD_LEFT);
        } else {
            $codigo = 'COT-005512';
        }
       return $codigo;
    ...
    public function createFromRecepcion($id)
    {
        ...
        return view('modules.cotizaciones.editCoti', compact('recepcion',"codigo"));
    }
    ...
    public function generarPdf($id)
    {
        ...
        $data = [
            ...
            'fecha' => $cotizacion->fecha ? \Carbon\Carbon::parse($cotizacion->fecha)->format('d/m/Y') : now()->format('d/m/Y'),
            'cod_cotizar' => $cotizacion->cod_cotizar,
        ...
        return $pdf->stream('cotizacion' . $cotizacion->recepcion->numero_recepcion . '.pdf');
    }
}
